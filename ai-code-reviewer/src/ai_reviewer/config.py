...
def load_config(config_path: Path | None = None) -> Config:
    ...
    if config_path.exists():
        # Encoding must be explicit: on Windows the locale default (GBK on zh-CN
        # systems) cannot decode UTF-8 configs containing non-ASCII comments.
...
def _parse_config(raw: dict[str, Any]) -> Config:
    ...
    if not agents:
        # Keep the complete five-role order used by review.py. The CLI's
        # --agents value still selects the first N roles, so the default
        # invocation remains three agents while AI_REVIEWER_AGENTS=4/5 can
        # activate the extra performance/style roles without a YAML file.
        agents = [
            AgentConfig(
            ...
            AgentConfig(
                ...
                model=anthropic.default_model,
                focus_areas=["consistency", "patterns", "architecture"],
            ),
            AgentConfig(
    ...
    # Per-role model division of labor via environment: AI_REVIEWER_MODEL_<ROLE>,
    # where ROLE is the agent name with the "-reviewer" suffix removed and
    # upper-cased (SECURITY / LOGIC / PATTERNS / PERFORMANCE / STYLE /
    # AUTHENTICATION), e.g. AI_REVIEWER_MODEL_LOGIC="some-model". Roles not
    # mentioned keep their resolved model (AI_REVIEWER_MODEL / ANTHROPIC_MODEL
    # via default_model);
    # an explicit agents[].model from the config file always wins.
    for agent in agents:
        if agent.name in yaml_agent_names:
            continue
        role_hint = agent.name.removesuffix("-reviewer").upper()
        env_model = os.environ.get(f"AI_REVIEWER_MODEL_{role_hint}", "")
        if env_model:
