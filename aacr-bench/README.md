...
The evaluation framework is in the [`evaluation/`](evaluation/) directory, which provides a unified pipeline: **data loading → review execution → evaluation**. It supports four reviewers: **OpenCodeReview (OCR)**, **Claude Code**, **Codex**, and **AI Code Reviewer (ai-reviewer, Calimero multi-agent)**. See [`evaluation/README.md`](evaluation/README.md) for detailed setup and usage.
